...
export class PresidentsController {
  ...
  clearSelection(): void {
    ...
    this.desiredCam.set(0, 1.2, 4.2)
    this.desiredLook.set(0, 0.9, 0)
  ...
  private tick = (): void => {
    ...
    if (this.phase === 'explore' && !this.selectedId) {
      ...
      this.desiredCam.set(
        ...
        1.25 + Math.sin(this.idleOrbit * 0.5) * 0.15,
  ...
  private focusCameraOn(id: string): void {
    ...
    if (!card) return
    const pos = card.anim.basePosition.clone()
