...
import { RedCastleWorld } from '@/ar/red-castle/RedCastleWorld'
import { redCastleMembers, getRedCastleMemberIndex } from '@/data/redCastleMembers'
...
export interface RedCastleControllerHooks {
  onPhase?: (phase: RedCastlePhase) => void
...
/**
 * Free WebGL board experience (desktop demos + non-AR mobile).
 * Raycasting, cinematic camera, intro timeline.
 */
export class RedCastleController {
  private renderer: THREE.WebGLRenderer
  private scene = new THREE.Scene()
  private camera: THREE.PerspectiveCamera
  private controls: OrbitControls
  private world = new RedCastleWorld()
  private raycaster = new THREE.Raycaster()
  private pointer = new THREE.Vector2()
  private clock = new THREE.Clock()
  private running = false
  private raf = 0
  private phase: RedCastlePhase = 'boot'
  private phaseTime = 0
  private hoveredId: string | null = null
  private selectedId: string | null = null
  private selectedIndex = -1
  private hooks: RedCastleControllerHooks = {}
  ...
  setHooks(hooks: RedCastleControllerHooks): void {
    this.hooks = hooks
  }
  ...
  selectById(id: string): void {
    const index = getRedCastleMemberIndex(id)
    if (index < 0) return
  ...
  selectByIndex(index: number): void {
    const clamped = (index + redCastleMembers.length) % redCastleMembers.length
    const member = redCastleMembers[clamped]
  ...
  private setPhase(phase: RedCastlePhase): void {
    this.phase = phase
    this.phaseTime = 0
