...
export function ExplorePanel({
  ...
  const [legendsTab, setLegendsTab] = useState<'profiles' | 'tactics'>('profiles')
          ...
          {section === 'legends' && (
            <div className="space-y-4">
              <div className="flex gap-2">
                {(['profiles', 'tactics'] as const).map((tab) => (
                  <button
                    key={tab}
                    type="button"
                    onClick={() => {
                    ...
                    className={`flex-1 rounded-xl border px-3 py-2 font-title text-[11px] tracking-[0.2em] transition ${
                      legendsTab === tab
                        ? 'border-pitch-gold/60 bg-pitch-gold/15 text-pitch-gold'
                        : 'border-white/10 bg-white/5 text-white/50'
                    }`}
                  >
                    {tab === 'profiles' ? copy.legendsTabProfiles : copy.legendsTabTactics}
                  </button>
                ))}
              </div>

              {legendsTab === 'tactics' && (
                <TacticsBoard onClose={() => setLegendsTab('profiles')} />
              )}
            </div>
          )}
