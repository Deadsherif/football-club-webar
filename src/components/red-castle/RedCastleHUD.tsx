import type { RedCastlePhase } from '@/ar/red-castle/RedCastleController'
import type { RedCastleMember } from '@/data/redCastleMembers'
import { t } from '@/i18n'
import { audio } from '@/services/audioService'

interface RedCastleHUDProps {
  phase: RedCastlePhase
  roleLabel: string
  selected?: RedCastleMember
  showBanner: boolean
  onBack: () => void
  onPrev: () => void
  onNext: () => void
  ...
  return (
    <>
      ...
      {phase === 'explore' && !selected && (
        <div className="pointer-events-none absolute inset-x-0 bottom-[max(1.5rem,env(safe-area-inset-bottom))] z-30 flex flex-col items-center gap-1 text-center text-[11px] tracking-[0.14em] text-white/70">
          <p>{copy.redCastleHint}</p>
          <p className="text-white/55">{copy.redCastleNavigate}</p>
        </div>
      )}
    </>
  )
